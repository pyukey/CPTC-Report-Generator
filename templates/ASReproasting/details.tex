...
When the TGT is requested, a session key is also received from the KDC (Key Distribution Center) on the Domain Controller, which is encrypted with a user's password. Pre-authentication is something that requires a TGT requestor to prove their identity before the KDC will issue a ticket. 
